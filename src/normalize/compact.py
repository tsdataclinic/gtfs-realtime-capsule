...
from norm_utils import update_last_processed_timestamp, validate_date, get_last_processed_timestamp, load_config, check_config
...
def compact_files(s3, s3_fs, normalized_prefix, start_date, normalize_state_file, compact_state_file):
    dataset_bucket, dataset_key = normalized_prefix.replace("s3://", "").split(
        "/", 1)
    normalized_state_bucket, normalized_state_key = normalize_state_file.replace("s3://", "").split(
        "/", 1)
    compact_state_bucket, compact_state_key = compact_state_file.replace("s3://", "").split(
        "/", 1)

    last_normalized_time = get_last_processed_timestamp(
        s3,
        normalized_state_bucket,
        normalized_state_key
    )
    if not last_normalized_time:
        LOGGER.info(f"No last normalized time for {normalized_prefix} at {normalize_state_file}. Sleeping")
        return
    ...
    if last_compact_time:
        ...
        LOGGER.info(
            f"No compact state information found at {compact_state_file},"
    ...
    while cur_processing < last_normalized_time.date():
        for message_type in ["trip-updates", "vehicles", "alert"]:
            ...
            if _s3_prefix_exists(dataset_bucket, os.path.join(dataset_key, message_type), s3):
                LOGGER.info(f"Compacting partition {cur_processing.date()} for {message_type} in {message_type_prefix}")
                compact(s3_fs, message_type_prefix, cur_processing)
        cur_processing += dt.timedelta(days=1)
    for message_type in ["trip-updates", "vehicles", "alert"]:
        message_type_prefix = os.path.join(normalized_prefix, message_type)
        if _s3_prefix_exists(dataset_bucket, os.path.join(dataset_key, message_type), s3):
            LOGGER.info(f"Compacting partition {cur_processing.date()} for {message_type} in {message_type_prefix}")
            compact(s3_fs, normalized_prefix, cur_processing)
    LOGGER.info(
        f"Updating last compacted timestamp to "
        f"last date processed: {cur_processing.timestamp()}"
    )
    update_last_processed_timestamp(s3, compact_state_bucket, compact_state_file, cur_processing.timestamp())
...
def main(
    ...
    if not normalize_state_file:  # set default
        normalize_state_file = f"{s3_bucket_path}/state/normalize/{feed_id}"
    if not compact_state_file:  # set default
        compact_state_file = f"{s3_bucket_path}/state/compact/{feed_id}"
    ...
    while True:
        compact_files(
            s3=s3,
            s3_fs=s3_fs,
            normalized_prefix=os.path.join(
                f"{s3_bucket_path}/{normalized_prefix}", feed_id
            ),
            start_date=start_date,
            normalize_state_file=normalize_state_file,
            compact_state_file=compact_state_file,
        )
        time.sleep(interval)
